...
void GLViewNewModule::onKeyDown(const SDL_KeyboardEvent& key)
{
	...
	{
		WOPhysicX* wheeledCarWO = WOPhysicX::New(ManagerEnvironmentConfiguration::getSMM() + "/models/rcx_treads.wrl", Vector(1, 1, 1), MESH_SHADING_TYPE::mstFLAT);
		wheeledCarWO->setPosition(Vector(50, 15, 75));
		wheeledCarWO->renderOrderType = RENDER_ORDER_TYPE::roOPAQUE;
		worldLst->push_back(wheeledCarWO);
		actorLst->push_back(wheeledCarWO);
		wheeledCarWO->setEngine(this->physEngine);
		physx::PxRigidDynamic* da = this->physEngine->createDynamic(wheeledCarWO);
		this->physEngine->addToScene(da);

		if (this->client->isTCPSocketOpen()) {
			this->nmc->setCamPos(cam->getPosition());
			this->nmc->setCamLookDir(cam->getLookDirection());
			this->nmc->setObjPos(wheeledCarWO->getPosition());
			this->nmc->setRotateZ(0.0f);
			this->nmc->setActorIndex(actorLst->getIndexOfWO(wheeledCarWO));
			this->nmc->setModelPath(ManagerEnvironmentConfiguration::getSMM() + "/models/rcx_treads.wrl");
			this->nmc->setDisplayMatrix(wheeledCarWO->getModel()->getDisplayMatrix());
			this->client->sendNetMsgSynchronousTCP(*this->nmc);
		}
	}
...
void Aftr::GLViewNewModule::loadMap()
{
	...
	//wo->setPosition(Vector(50, 15, 0));
	//wo->getModel()->setScale(Vector(2, 2, 2));
